# UZN TECH — Yol Haritası (v19)

Bu belge v18'in yerine geçer.

## v19 — Sitenin "anlaşılırlığı" turu (3 katman, hepsi uygulandı)

Kullanıcı isteği: "sayfanın anlaşılır olmasını istiyorum". Üç katmanda
ele alındı, kullanıcı hepsini aynı turda istedi.

### Katman 1 — Kesin tutarsızlıklar (düzeltildi)
- **Nav etiketi uyuşmazlığı:** `index.html`'de U2 linki "Hakkımda"
  yazıyordu, hedef sayfa (`kurumsal.html`) ve diğer 5 sayfa "Kurumsal"
  diyordu. `index.html`'deki 5 görünür yer (site-nav, mobile-nav,
  side-rail, quick-tile başlığı, footer linki, cmdk arama öğesi)
  "Kurumsal" olarak düzeltildi.
- **Eksik SSS linki:** `kurumsal.html` + 4 track sayfasının site-nav'ında
  U3'ten direkt U5'e atlıyordu, U4 (SSS) eksikti. 5 sayfaya da
  `index.html#sss` linkiyle U4 SSS eklendi. (Footer'da zaten vardı,
  sadece üst nav'da eksikti.)

### Katman 3 — Terim tutarlılığı (düzeltildi)
- Nav'da "Hizmetler" derken bölüm başlığı "Çalışmalar & Yetkinlikler"
  ve eyebrow etiketi "U1 — ÇALIŞMALAR" diyordu — aynı bölüm için iki
  farklı isim. Nav'daki TÜM "Hizmetler" görünür metinleri (site-nav,
  side-rail, quick-tile, footer, cmdk — index.html'de 5 yer, diğer 5
  sayfada 1'er yer = toplam 10 yer) "Çalışmalar" olarak değiştirildi.
  `href="#hizmetler"` / `id="hizmetler"` (URL parçası) BİLİNÇLİ OLARAK
  değiştirilmedi — JS (`goToTrack`, IntersectionObserver, side-rail
  active-state) buna bağlı, gereksiz risk almamak için sadece görünen
  metin değişti.

### Katman 2 — İçerik netliği (uygulandı)
- **Kimlik ayrımı (kullanıcı kararı: "ikisini net ayıralım"):**
  `kurumsal.html`'deki `about-text` ikiye bölündü:
  1. Kurumsal ton paragrafı — "UZN TECH & SOFTWARE ne yapar" (3. şahıs/
     şirket diliyle yeniden yazıldı, eski 2. paragrafın içeriğine
     dayanıyor, yeni bir iddia eklenmedi).
  2. Ayrı, görsel olarak belirgin **"Kurucudan"** alıntı bloğu
     (`blockquote.founder-note` — sol kenarlıklı, tonu farklı kutu,
     "— Fatih Uzner" imzasıyla) — eski 1. paragrafın ilk şahıs
     içeriği buraya taşındı. style.css'e `.founder-note` /
     `.founder-note-label` / `.founder-note cite` eklendi.
- **Placeholder şeffaflığı:** "Akış" bölümünün başlığının altına
  `.section-placeholder-note` (zaten var olan sınıf, projeler
  bölümünde kullanılıyordu) ile "Site yeni yayında — ilk gerçek
  güncellemeler ve duyurular burada görünmeye başlayacak." notu
  eklendi — "Yakında" kartlarının neden boş göründüğü artık açık.

### Bilinçli olarak yapılMAYAN
- **CTA hiyerarşisi:** İncelendi — `btn-primary` ("Bir Proje
  Konuşalım") / `btn-ghost` ("Yetkinlikleri Gör") ayrımı zaten görsel
  önceliği taşıyor; büyük bir yeniden düzenleme gerekmediği
  değerlendirildi, dokunulmadı.
- **U0–U5 / REF.01 HUD referans etiketleri:** Kullanıcı önceki turda
  "şirket hissi kalsın" demişti (kimlik sorusunun diğer seçenekleri
  arasında bu seçilmedi ama HUD estetiği zaten sitenin karakteri) —
  jargon riski not edildi ama görsel kimliğin parçası olduğu için
...
aynı turda uygulanacak:

**a) Topology düğümleri → sayfa linki**
Her `node-panel` içine, `tool-tags` bloğunun altına, siteye zaten
hakim olan HUD/şematik dile uygun bir CTA linki eklenecek (mono font,
küçük ok, hover'da topology hatlarındaki "sinyal" rengiyle uyumlu —
örn. `→ yazilim.html'de devamını gör`). Accordion (`<details>`) yapısı
KORUNACAK — düğümü tamamen linke çevirmek yerine (zengin önizlemeyi
kaybettirir) mevcut panelin içine ek bir bağlantı satırı eklenecek.
Eşleme: mekanik → `mekanik-otomasyon.html`, yazılım → `yazilim.html`,
siber → `siber-guvenlik.html`, mekatronik (hub) → `mekatronik.html`.

**b) Hero'daki track filtre butonları (Yazılım/Mekanik/Siber)**
Şu an "Yazılım" butonu hiçbir şeyi filtrelemiyor (P01-P03 taşındığı
için). Filtre davranışı TAMAMEN kaldırılacak. Yeni davranış: butona
tıklanınca sayfa `#hizmetler`e (topology haritasına) kayacak VE
ilgili düğüm otomatik `<details open>` ile açılacak. Kullanıcı
oradan (a)'daki linkle ilgili track sayfasına geçebilecek.
DİKKAT: `#projeler` listesindeki mekanik/siber `is-planned-project`
satırlarının `data-track` filtrelenebilirliği bu değişiklikten
etkilenmemeli — sadece hero butonlarının davranışı değişiyor, proje
listesi filtre mantığına dokunulmayacak (varsa ayrı bir yerde kalıyor
olabilir, uygulama turunda kontrol edilecek).

**c) Track sayfalarına ek erişim noktası**
Quick-tiles'a dokunulmayacak (4 kutu zaten dolu, kalabalıklaşmasın).
...
# v17 (önceki sürüm, aynen korunuyor)
